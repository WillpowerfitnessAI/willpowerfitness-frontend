...
function buildPlan(a) {
  const days = String(a.schedule || "").match(/\d+/)?.[0] ? parseInt(String(a.schedule).match(/\d+/)[0], 10) : 3;
  ...
  const tweaks = [];
  if (/knee/.test(cts)) {
    tweaks.push("knee-friendly lower: box squat to parallel, RDLs, sled push/pull, cycling/rowing");
  }
  if (/shoulder/.test(cts)) {
    tweaks.push("shoulder-safe upper: landmine press, neutral-grip DB press/rows, cable work, avoid deep dips");
  }
  ...
  if (days >= 4) {
    sample.push("Day 4 – Lower pump: Hack or goblet squat 3×10, hip hinge 3×10, ham curl 3×12, calves");
  }
  if (days >= 5) {
    sample.push("Day 5 – Upper strength: DB bench 5×5, weighted row 5×5, dips/push-ups, accessories");
  }

  // tone in Stuff's voice
...
export default function Consult() {
  ...
  function persistLeadMinimal(a){
    try {
      ...
      // Optional: beacon to backend for minimal (name/email) only
      navigator.sendBeacon?.("https://api.willpowerfitnessai.com/api/lead-min",
        JSON.stringify({ name:a.name, email:a.email, source:"consult-exit" }));
    } catch {}
  }

  async function sendFullIntake(intent){
    // full intake only when user starts trial or joins
        ...
        {step >= questions.length && (
          <div style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:8}}>
            ...
            <a href={trialUrl || "#"}
              ...
              <button style={{padding:"0.85rem 1rem",borderRadius:12,border:"1px solid #1f1f1f",background:"#1a1a1a",color:"#fff",cursor:"pointer",opacity:disabledTrial?.8:1}}>
                Start Trial
              </button>
            </a>
